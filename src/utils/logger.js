...
module.exports = class Logger {
	static log (content, type = "log") {
		...
		const webhookEnabled = String(process.env.LOG_WEBHOOKS_ENABLED || 'false').toLowerCase() === 'true';
    ...
    try {
      ...
      console.error('logger sanitize error:', e && (e.message || e));
		...
		case "log": {
 			...
 			// send to guild webhook only when explicitly enabled
 			if (webhookEnabled) {
	 			try {
		...
		case "error": {
 			...
 			// send to error webhook only when explicitly enabled
 			if (webhookEnabled) {
	 			try {
