...
module.exports = {
    ...
    run: async (client, interaction) => {
      return await client.errorHandler.executeWithErrorHandling(interaction, async (interaction) => {
        ...
        const tracks = Array.isArray(player?.queue?.tracks) ? player.queue.tracks.slice() : [];
        const qSize = tracks.length;

        // Validate positions
        if (from < 1 || from > qSize) {
          const embed = new EmbedBuilder()
            .setColor(interaction.client?.embedColor || '#ff0051')
            .setDescription(`${no} From position must be a number between \`1\` and \`${qSize}\``);
          return await safeReply.safeReply(interaction, { embeds: [embed] });
        }
        ...
        if (to < 1 || to > qSize) {
          const embed = new EmbedBuilder()
            .setColor(interaction.client?.embedColor || '#ff0051')
            .setDescription(`${no} To position must be a number between \`1\` and \`${qSize}\``);
          return await safeReply.safeReply(interaction, { embeds: [embed] });
        }
        ...
        try {
          ...
          await player.queue.splice(0, player.queue.tracks.length, newQueue);
