...
module.exports = {
  ...
  execute: async (message, args, client) => {
    ...
    if (!user) return sendResponse(message, "Please mention someone to slap! ??");
    if (!author) return sendResponse(message, "Unable to resolve command user.");
    if (user.id === author.id) return sendResponse(message, "You can't slap yourself! ??");

    const randomGif = await getActionGif("slap");
    const embed = new EmbedBuilder()
      .setColor("#ff6b6b")
      .setDescription(`${user.username} just received a playful slap from ${author.username}! ??`)
      .setImage(randomGif);
