...
module.exports = {
  ...
  execute: async (message, args, client) => {
    ...
    if (!user) return sendResponse(message, "Please mention someone to high five! ?");
    if (!author) return sendResponse(message, "Unable to resolve command user.");
    if (user.id === author.id) return sendResponse(message, "You can't high five yourself! ??");

    const randomGif = await getActionGif("highfive");
    const embed = new EmbedBuilder()
      .setColor(client?.embedColor || "#ff0051")
      .setDescription(`${author.username} and ${user.username} high five! That's awesome! ?`)
      .setImage(randomGif);
