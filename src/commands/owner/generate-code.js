  ...
  execute: async (message, args, client, prefix) => {
    ...
    if (!typeArg || !["user", "server", "guild"].includes(typeArg)) {
      const embed = new EmbedBuilder()
        .setColor(embedColor)
        .setTitle(`${getEmoji("error")} Invalid Arguments`)
        .setDescription([
    ...
    if (!duration) {
      const embed = new EmbedBuilder()
        .setColor(embedColor)
        .setTitle(`${getEmoji("error")} Invalid Duration`)
        .setDescription(`${no} Use: \`30m\`, \`2h\`, \`7d\`, \`1y\`, or \`permanent\`.`);
      return message.reply({ embeds: [embed], allowedMentions: { repliedUser: false } });
    }
    ...
    try {
      ...
      const embed = new EmbedBuilder()
        .setColor(embedColor)
        .setTitle(`${getEmoji("premium")} Premium Code Generated`)
        .setDescription(`${ok} New redeem code has been created.`)
        .addFields(
        ...
        .setFooter({ text: "Redeem with /redeem or prefix redeem command" });
      ...
      const embed = new EmbedBuilder()
        .setColor(embedColor)
        .setTitle(`${getEmoji("error")} Code Generation Failed`)
        .setDescription(`${no} Error creating code: ${error?.message || "Unknown error"}`);
      return message.reply({ embeds: [embed], allowedMentions: { repliedUser: false } });
    }
  }
};
